...
Board.boardUpdateProcess = function(idx, imgurl, mainId, subId, title, contents, result){
    ...
    // 빈값 체크
    if(imgurl !== ""){
        update = "UPDATE board set image=?, title=?, content=?, modidate=now() where idx=? and main_Id=? and sub_Id=?;";
    }else{
        update = "UPDATE board set title=?, content=?, modidate=now() where idx=? and main_Id=? and sub_Id=?;";
    }
