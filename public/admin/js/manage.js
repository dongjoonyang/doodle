...
function fnCategoryPopList(){
    ...
    .done(function(json){
        ...
        // 카테고리 타이틀 명 
        $("label[for=categoryOwn]").text(json.rows1[0].main_title);
        $("label[for=categoryTwo]").text(json.rows1[1].main_title);
        ...
        $("#categoryNewBtn").on("click", function(){
            ...
            $(".generate-item-del").on("click", function(){
                $(".modal__generate_new").remove();
                $("#categoryNameInput").prop("readonly", true);
                $("#categoryNameInput-error").css("display", "none");
            })
        });
    ...
    if(json.rows != ''){
        ...
        for(var i = (json.page * json.page_num) - json.page_num; i < (json.page * json.page_num); i++) {
            if(i > json.length){
                ...
                listHtml += "<td class='mng__table--center'>"+ data.main_title + "</td>";
                listHtml += "<td class='mng__table--center'>"+ data.sub_title +"</td>";
                listHtml += "</tr>"
            } 
        };
...
$(function() {
     ...
     * =======================================
     */
    $(".mng-category__all button[id^='cateTitModBtn']").on("click", function(){
        $(this).addClass("display-none");
        $(this).siblings().removeClass("display-none");
        $(this).siblings(".mng-category__input").focus();
    });

    // refactoring
    $("#cateTitModBtn1").on("click", function(){
        $("#mngCategoryInput2").addClass("display-none");
        $(".apply.2").addClass("display-none");
        $("#cateTitModBtn2").removeClass("display-none");
    })

    $("#cateTitModBtn2").on("click", function(){
        $("#mngCategoryInput1").addClass("display-none");
        $(".apply.1").addClass("display-none");
        $("#cateTitModBtn1").removeClass("display-none");
    })

    /**
     * =======================================
     * 설  명 : 메인 카테고리 타이틀 변경 적용
     * =======================================
     */
    $(".mng-category__all button[id^='applyGoBtn']").on("click", function(){
        let mainId = $(this).data("mainId");
        let parameter = $("#categoryTitleForm").serializeObject();        
        parameter.mngCategoryId = mainId;

        if(mainId === 1){
            if(parameter.mngCategory1 === ""){
                alert("타이틀을 입력해주세요");
                return ;
            }else{
                mngCategoryAjax();
        ...
        function mngCategoryAjax(){
            $.ajax({
                type : "POST",
                url : "/admin/manage/manageCategoryTitleUpdateProcess",
                dataType : "JSON",
                data : parameter
            })
            .done(function(json){
                $("#mngCategoryInput" + mainId).addClass("display-none");
                $("#cateTitModBtn" + mainId).removeClass("display-none");
                $("#cateTitModBtn" + mainId).next().addClass("display-none");
                ...
                .done(function(json){
                    $(`.mng-category__all a[data-main-id='${mainId}'] span`).text(mainId === 1 ? json.rows1[0].main_title : json.rows1[1].main_title);
                    $(".mng-category__all a span").attr("title", mainId === 1 ? json.rows1[0].main_title : json.rows1[1].main_title)
                })
                .fail(function(request, status, error){
                    console.log("삭제 후 서브 카테고리 게시판 목록 불러오기 Ajax failed");
                });
            ...
            .fail(function(xhr, status, errorThrown){
                console.log("카테고리 타이틀 변경 Ajax failed")
            })
        }
    });
     ...
     * =======================================
     */
    $(".mng-category__all button[id^='applyCancleBtn']").on("click", function(){
        $(this).parent().addClass("display-none");
        $(this).parent().siblings("input").addClass("display-none");
        $(this).parent().siblings("button").removeClass("display-none");
    });
